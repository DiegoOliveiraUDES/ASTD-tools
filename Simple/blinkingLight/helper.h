...
class Types {
 ...
 public:
    ...
    static bool get_bool(const std::string s) { return (s != "0") || (s != "false"); }
...
class Clock {
public:
    static void reset_clock(std::time_t& clock){
        clock = std::chrono::duration_cast<std::chrono::nanoseconds>((std::chrono::system_clock::now().time_since_epoch())).count();;
    }
};